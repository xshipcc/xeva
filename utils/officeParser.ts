    ...
    extractFiles(file, (x) => !!x.match(config.ignoreNotes ? slidesRegex : allFilesRegex))
      .then((files: ExtractedFiles[]) => {
        ...
        files.sort((a, b) => {
          ...
          return aNumber - bNumber || Number(a.filename.includes('notes')) - Number(b.filename.includes('notes'))
        })

        // Verify if atleast the slides xml files exist in the extracted files list.
        if (files.length == 0 || !files.map((file) => file.filename).some((filename) => filename.match(slidesRegex)))
          handleError(ERRORMSG.fileCorrupted(file.name), config.outputErrorToConsole)
...
export function parseExcel(file: File, config: Partial<OfficeParserConfig>): Promise<string | File> {
  ...
  return new Promise((resolve, reject) => {
    extractFiles(
      ...
      .then((files: ExtractedFiles[]) => {
        // Verify if atleast the slides xml files exist in the extracted files list.
        if (files.length == 0 || !files.map((file) => file.filename).some((filename) => filename.match(sheetsRegex)))
          handleError(ERRORMSG.fileCorrupted(file.name), config.outputErrorToConsole)
        ...
        return {
          ...
          sharedStringsFile: files.filter((file) => file.filename == stringsFilePath)[0],
        }
      })
      ...
      .then(async (xmlContentFilesObject) => {
        ...
        const sharedStringsXmlTNodesList =
          xmlContentFilesObject.sharedStringsFile != undefined
            ? parseXMLString(await xmlContentFilesObject.sharedStringsFile.data.text()).getElementsByTagName('t')
            : []
        ...
        for await (const drawingFile of xmlContentFilesObject.drawingFiles) {
          ...
          /** Find text nodes with a:p tags */
          const drawingsXmlParagraphNodesList = parseXMLString(drawingXmlContent).getElementsByTagName('a:p')
          /** Store all the text content to respond */
          responseText.push(
  ...
  return new Promise((resolve, reject) => {
    extractFiles(file, (x) => x == mainContentFilePath || !!x.match(objectContentFilesRegex))
      .then((files) => {
        ...
        return {
          mainContentFile: files.filter((file) => file.filename == mainContentFilePath)[0],
      ...
      .then(async (xmlContentFilesObject) => {
        ...
        xmlContentArray.forEach((content) => {
          ...
          const xmlTextNodesList = [
            ...Array.from(xmlContent.getElementsByTagName('*')).filter(
              (node) => allowedTextTags.includes(node.tagName) && !isInvalidTextNode(node.parentNode!),
            ),
          ]
          /** Store all the text content to respond */
          responseText.push(
        ...
        if (!config.ignoreNotes && config.putNotesAtLast) responseText = [...responseText, ...notesText]
...
export function parseOffice(file: File, config?: Partial<OfficeParserConfig>) {
  ...
  switch (file.type) {
    ...
    default:
      return handleError(ERRORMSG.extensionUnsupported(file.type), internalConfig.outputErrorToConsole)
  }
}
